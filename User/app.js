...
(() => {
  ...
  function initMatchedCommunity() {
    ...
    const postData = {
      ...
      dung: {
        author: "Dũng",
        content: "Đây là một số từ vựng thường gặp khi học tiếng Anh: curious, intelligent, confident, patient, creative, responsible, helpful, friendly, active, careful, generous, honest và polite.",
        likes: 540
      }
    };
...
(() => {
  ...
  function initCommunityGuestMode() {
    if (document.body.dataset.currentPage !== "community") return;

    const isLoggedIn =
      localStorage.getItem("lingoverseLoggedIn") === "true";

    if (isLoggedIn) {
      document.body.classList.remove("community-guest-body");
      return;
    }

    document.body.classList.add("community-guest-body");

    const composer = document.getElementById("openCreatePost");
    if (composer) {
    ...
    if (createOverlay) createOverlay.hidden = true;
    if (detailOverlay) detailOverlay.hidden = true;

    function requireLogin(message) {
      if (window.LingoPopup?.loginRequired) {
        window.LingoPopup.loginRequired(message);
        return;
      }

      const modal = document.getElementById("authModal");
      const modalMessage = document.getElementById("authModalMessage");
      const loginLink = document.getElementById("modalLoginLink");

      if (modalMessage) modalMessage.textContent = message;
      if (loginLink) {
        loginLink.href =
          `login.html?redirect=${encodeURIComponent("community.html")}`;
      }
      ...
      (event) => {
        const protectedAction = event.target.closest(
          [
            ...
            "[data-open-post]",
            "[data-report-post]",
            "[data-community-tab='favorite']",
            "[data-community-tab='following']"
          ].join(",")
        );
        ...
        let message =
          "Bạn cần đăng nhập để sử dụng chức năng cộng đồng.";

        if (protectedAction.matches("[data-like-post]")) {
          message = "Đăng nhập để yêu thích bài viết này.";
        } else if (protectedAction.matches("[data-open-post]")) {
          message = "Đăng nhập để xem và tham gia bình luận.";
        } else if (protectedAction.matches("[data-report-post]")) {
          message = "Đăng nhập để gửi báo cáo bài viết.";
        } else if (protectedAction.matches("[data-community-tab='favorite']")) {
          message = "Đăng nhập để xem các bài viết bạn đã yêu thích.";
        } else if (protectedAction.matches("[data-community-tab='following']")) {
          message = "Đăng nhập để xem bài viết từ những người bạn quan tâm.";
        } else if (protectedAction.id === "openCreatePost") {
          message = "Đăng nhập để tạo bài viết mới.";
        }
    ...
    document.getElementById("communityCommentForm")?.addEventListener(
      "submit",
      (event) => {
        event.preventDefault();
        event.stopImmediatePropagation();
        requireLogin("Đăng nhập để bình luận về bài viết.");
      },
      true
    );
  }

  initCommunityGuestMode();
})();
